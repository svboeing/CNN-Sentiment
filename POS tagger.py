...
# Extract words and embeddings from fasttext's output, skipping first line:
skip = False
words = []
vecs = []
...
# Form supervector from words, tags, suffixes and prefixes:
...
# Embedding lookup for words, tags, suffixes, prefixes from supervector:
...
X_test = tf.placeholder(tf.int32, [1, 24]) #batches
...
# Change batch_size to 1 because now tags are unknown:
batch_size = 1
