...
sent_train_reviews, sent_test_reviews = [], []

for i in range(len(sent_train_words_raw)):
    sent_train_reviews.append([vocab_to_int[imdb_id_to_w[_id]] for _id in sent_train_words_raw[i]
                               if imdb_id_to_w[_id] in unique])
    sent_test_reviews.append([vocab_to_int[imdb_id_to_w[_id]] for _id in sent_test_words_raw[i]
                              if imdb_id_to_w[_id] in unique])


sent_train_reviews = sequence.pad_sequences(sent_train_reviews, maxlen=sent_maxlen)
sent_test_reviews = sequence.pad_sequences(sent_test_reviews, maxlen=sent_maxlen)
# Only full batches on training:
#print(len(sent_train_reviews), len(sent_train_labels))
#print(sent_train_labels[0], sent_train_reviews[0])

sent_batch_size = len(sent_train_reviews) // num_global_batches
sent_train_reviews = sent_train_reviews[:num_global_batches * sent_batch_size]
sent_train_labels = sent_train_labels[:num_global_batches * sent_batch_size]
print(len(sent_train_reviews), len(sent_train_labels))
#print(len(sent_train_words), len(sent_train_labels))
...
CNN_net = CNNmodule.CNN(n_embedding, n_vocab, sent_max_features, sent_maxlen, n_filters,
...
def get_joint_batches(w2v_train_words, tagger_train_words, tagger_train_labels, sent_train_reviews, sent_train_labels):
    for i in range(num_global_batches):
        ...
        # cnn part:
        sent_x = sent_train_reviews[i * sent_batch_size:(i + 1) * sent_batch_size]
        ...
        batch_words = set(w2v_batch) & set(
            [word_id[word] for word in tagger_train_words[i*tagger_batch_size : (i+1)*tagger_batch_size]]) ## TODO CAREFUL WITH THAT BATCH_WORDS, EUGENE

        yield w2v_x, w2v_y, tagger_x, tagger_y, sent_x, sent_y, batch_words
...
with tf.Session() as sess:
    ...
    for e in range(1, epochs + 1):
        ...
        BATCH = get_joint_batches(w2v_train_words, tagger_train_words, tagger_train_labels, sent_train_reviews, sent_train_labels)

        counter = 0
        common_processed_words = set()
        for w2v_x, w2v_y, tagger_x, tagger_y, sent_x, sent_y, batch_words in BATCH:

            sess.run(CNN_net.train, feed_dict={CNN_net.X: sent_x, CNN_net.Y: sent_y})
            ...
            if e > 1 and counter % 1000 == 0:
                ...
                i = 0
                while i * sent_eval_batch_size < len(sent_test_reviews):
                    sent_x_test = sent_test_reviews[i * sent_eval_batch_size:(i + 1) * sent_eval_batch_size]
                    sent_y_test = sent_test_labels[i * sent_eval_batch_size:(i + 1) * sent_eval_batch_size]
                    i += 1
                    a = sess.run(CNN_net.batch_prediction, feed_dict={CNN_net.X: sent_x_test, CNN_net.Y: sent_y_test})
                ...
                F1_weighted = f1_score(y_true=sent_test_labels, y_pred=sent_prediction, average='weighted')
                # with open("cnn_accs.txt", "a") as f:
                #    f.write(str(F1))
                print("SENTIMENT F1 score: ", F1)
                print('SENTIMENT WEIGHTED F1 score: ', F1_weighted)
                ...
                for i in range(len(tagger_test_words)):
                    tagger_x_test = TAGGER_net.form_minibatch(tagger_test_words, tags_learned, i, 1, word_id, pref_id)
                    tagger_y_test = TAGGER_net.form_onehot_batch(tagger_test_labels, i, 1)


                    if encoded_tagger_eval == False:
                        logits = sess.run(TAGGER_net.test_logits,
                                          feed_dict={TAGGER_net.X: tagger_x_test, TAGGER_net.Y: tagger_y_test})
                    else:
                        logits = sess.run(TAGGER_encoder_test_logits,
                                          feed_dict={COMMON_enc.X: tagger_x_test[:, :7].flatten(),
                                                     TAGGER_priv_enc.X: tagger_x_test[:, :7].flatten(), TAGGER_net.X: tagger_x_test,
                                                     TAGGER_net.Y: tagger_y_test})
                    logits = logits[0]
